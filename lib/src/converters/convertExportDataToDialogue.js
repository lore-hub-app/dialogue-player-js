...
function createNode(node, dialogueNodes, contentBlocks, documents, options){
   ...
   for (const optionId of node.optionsIds) {
     ...
     convertedOptions.push(new DialogueNodeOption(neededOption.id, neededOption.text, nextNode))
   }
  ...
  return new DialogueNode(node.id, convertedContent, nextNode, convertedOptions);
}
